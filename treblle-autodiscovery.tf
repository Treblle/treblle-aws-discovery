...
resource "aws_iam_role_policy" "treblle_discovery_policy" {
  name = "TreblleDiscoveryPolicy"
...
# Lambda Function
resource "aws_lambda_function" "api_gateway_discovery_function" {
  function_name = "treblle-api-gateway-discovery"
  ...
  environment {
    variables = {
      TREBLLE_SDK_TOKEN = var.treblle_sdk_token
      SCAN_REGIONS      = join(",", var.region_list)
    }
  }
...
resource "aws_cloudwatch_event_target" "lambda_target" {
  ...
  target_id = "APIGatewayDiscoveryTarget"
  arn       = aws_lambda_function.api_gateway_discovery_function.arn
}
...
resource "aws_lambda_permission" "lambda_invoke_permission" {
  ...
  function_name = aws_lambda_function.api_gateway_discovery_function.function_name
...
resource "aws_cloudwatch_log_group" "log_group" {
  name              = "/aws/lambda/${aws_lambda_function.api_gateway_discovery_function.function_name}"
...
output "lambda_function_arn" {
  ...
  value       = aws_lambda_function.api_gateway_discovery_function.arn
}

output "scan_regions" {
  description = "Regions being scanned"
  value       = join(",", var.region_list)
}
...
Scan Regions: ${join(",", var.region_list)}
...
Deploy this configuration in each target account you want to monitor:
terraform apply -var="treblle_sdk_token=TOKEN" -var="region_list=[\"us-east-1\",\"us-west-2\"]"
EOF
}